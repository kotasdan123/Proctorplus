...
export const ResultView: React.FC<ResultViewProps> = ({ room, result, onReturn }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
      <div className={`w-full max-w-lg bg-slate-900 border rounded-3xl p-8 shadow-2xl text-center relative overflow-hidden ${
        isTerminated ? 'border-red-600/50 shadow-red-950/30' : 'border-slate-800'
      }`}>
        {/* Glow accent */}
        ...
        <div className={`w-20 h-20 rounded-3xl mx-auto flex items-center justify-center mb-6 shadow-xl ${
          isTerminated
            ? 'bg-red-500/20 border-2 border-red-500/40 text-red-400 shadow-red-500/20'
            : isTimeExpired
        ...
        <span className={`text-[11px] font-bold tracking-widest uppercase px-3 py-1 rounded-full border ${
          ...
          {isTerminated ? 'SESSION TERMINATED • EJECTED' : isTimeExpired ? 'TIME EXPIRED' : 'EXAMINATION COMPLETED'}
        </span>

        <h1 className="text-2xl sm:text-3xl font-extrabold text-white mt-4 mb-2 tracking-tight">
          {isTerminated
            ? 'You Were Ejected From The Exam'
            : isTimeExpired
            ? 'Your Time Has Ended'
        ...
        <p className="text-sm text-slate-400 mb-8 max-w-sm mx-auto leading-relaxed">
          {isTerminated
            ? `Your session was terminated because the security violation threshold was reached (${result.violations} violations). This incident is recorded in the proctor audit log.`
            : isTimeExpired
        ...
        <div className="grid grid-cols-2 gap-3 text-left mb-8">
          ...
          <div className="p-3.5 rounded-2xl bg-slate-800/50 border border-slate-700/60">
            ...
            <strong className={`text-sm font-semibold ${
              isTerminated ? 'text-red-400' : result.violations > 0 ? 'text-amber-400' : 'text-emerald-400'
            }`}>
              {result.violations} violation{result.violations === 1 ? '' : 's'}
              {isTerminated && ' (Limit reached)'}
            </strong>
          </div>
        </div>
